...
import { parseAmneziaWgConf } from '@/lib/mihomo/parse-amnezia-wg'
...
interface AmneziaWgImportProps {
  onAddProxies: (proxies: MihomoProxy[]) => void
  onBuild: () => void
}

export function AmneziaWgImport({ onAddProxies, onBuild }: AmneziaWgImportProps) {
  ...
  async function processFiles(files: FileList | null): Promise<void> {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const baseName = file.name.replace(/\.conf$/i, '').trim() || 'wireguard'
        const proxies = parseAmneziaWgConf(text, { baseName })
        if (proxies.length) {
          allProxies.push(...proxies)
        } else {
          errors.push(file.name + ': ' + t('mihomo.amneziaWgNoPeer'))
        }
      } catch (e) {
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-base">{t('mihomo.amneziaWgTitle')}</CardTitle>
        <p className="text-sm text-muted-foreground">{t('mihomo.amneziaWgHint')}</p>
      </CardHeader>
      <CardContent className="space-y-4 pt-0">
        <input
          ...
          className="sr-only"
          aria-label={t('mihomo.amneziaWgTitle')}
        />
        <div
          className={cn(
        ...
        >
          {t('mihomo.amneziaWgDrop')}
        </div>
        {lastResult && (
          <div className="space-y-2">
            {lastResult.count > 0 ? (
              <span className="text-sm text-muted-foreground">
                {t('mihomo.amneziaWgAdded', {
                  count: lastResult.count,
                  files: lastResult.files,
                })}
              </span>
            ) : (
              <span className="text-sm text-muted-foreground">{t('mihomo.amneziaWgNoPeer')}</span>
            )}
